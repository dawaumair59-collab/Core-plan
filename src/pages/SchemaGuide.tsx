...
CREATE POLICY "subscriptions_owner_all" ON public.subscriptions FOR ALL USING (auth.uid() = user_id);`
...
const tables = [
  {
    ...
    color: 'orange',
    icon: '🏪',
    fields: ['id (uuid pk)', 'user_id → auth.users', 'name, slug (unique)', 'description, logo_url', 'address, phone, email', 'cuisine_type', 'is_active', 'created_at, updated_at'],
    rls: 'Owner: full CRUD | Public: read active',
  },
  {
    name: 'menu_categories',
    color: 'yellow',
    icon: '📂',
    fields: ['id (uuid pk)', 'restaurant_id → restaurants', 'name, description', 'display_order', 'is_active', 'created_at, updated_at'],
    rls: 'Owner via restaurant | Public: read active',
  },
  {
    name: 'menu_items',
    color: 'green',
    icon: '🍽️',
    fields: ['id (uuid pk)', 'category_id → menu_categories', 'restaurant_id → restaurants', 'name, description, price', 'image_url, allergens', 'nutritional_info (jsonb)', 'is_available, is_featured', 'display_order', 'created_at, updated_at'],
    rls: 'Owner via restaurant | Public: read available',
  },
  {
    name: 'videos',
    color: 'blue',
    icon: '🎥',
    fields: ['id (uuid pk)', 'menu_item_id → menu_items', 'restaurant_id → restaurants', 'title, description', 'video_url, thumbnail_url', 'duration_seconds', 'view_count', 'is_published', 'created_at, updated_at'],
    rls: 'Owner via restaurant | Public: read published',
  },
  {
    name: 'analytics',
    color: 'purple',
    icon: '📊',
    fields: ['id (uuid pk)', 'restaurant_id → restaurants', 'event_type', 'menu_item_id → menu_items', 'video_id → videos', 'session_id, metadata (jsonb)', 'created_at'],
    rls: 'Owner read | Anyone can insert (tracking)',
  },
  {
    name: 'subscriptions',
    color: 'pink',
    icon: '💳',
    fields: ['id (uuid pk)', 'user_id → auth.users', 'restaurant_id → restaurants', 'plan (free/starter/pro/enterprise)', 'status (active/trialing/…)', 'current_period_start/end', 'cancel_at_period_end', 'stripe_subscription_id', 'stripe_customer_id', 'created_at, updated_at'],
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-zinc-400 mt-1">Full relational schema with RLS policies for your Supabase project.</p>
      </div>

      {/* Table cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {tables.map((t) => (
          <div key={t.name} className={`rounded-xl border p-5 ${colorMap[t.color]}`}>
            ...
            <div className={`text-xs rounded-lg px-2 py-1.5 ${badgeMap[t.color]}`}>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
        <h2 className="font-semibold mb-4 text-zinc-300">Relationships</h2>
        <div className="font-mono text-xs text-zinc-400 space-y-1 leading-relaxed">
          <div>auth.users ──▶ restaurants (user_id)</div>
          <div>auth.users ──▶ subscriptions (user_id)</div>
          <div className="pl-4">restaurants ──▶ menu_categories (restaurant_id)</div>
          <div className="pl-8">menu_categories ──▶ menu_items (category_id)</div>
          <div className="pl-4">restaurants ──▶ menu_items (restaurant_id)</div>
          <div className="pl-8">menu_items ──▶ videos (menu_item_id)</div>
          <div className="pl-4">restaurants ──▶ videos (restaurant_id)</div>
          <div className="pl-4">restaurants ──▶ analytics (restaurant_id)</div>
          <div className="pl-4">restaurants ──▶ subscriptions (restaurant_id)</div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900">
        <div className="flex items-center justify-between px-5 py-3 border-b border-zinc-800">
          <span className="text-sm font-medium text-zinc-300">schema.sql — paste into Supabase SQL Editor</span>
          <button
            onClick={copy}
        ...
        <pre className="p-5 overflow-x-auto text-xs text-zinc-400 leading-relaxed max-h-96">
          {SCHEMA_SQL}
        </pre>
      </div>
    </div>
